...
async fn test() -> Result<(), Error> {
    ...
    // `--instance` lives on the parent `cache` subcommand, not on the
    // leaf — `cache --instance <NAME> <leaf>`, never
    // `cache <leaf> --instance <NAME>`.
    let cache_prefix: &[&str] = &["cache", "--instance", instance.as_str()];

    let populate_args: Vec<&str> = [cache_prefix, &["populate"]].concat();
    let push_args: Vec<&str> = [cache_prefix, &["push"]].concat();
    let reset_args: Vec<&str> = [cache_prefix, &["reset", "--force"]].concat();
    let pull_args: Vec<&str> = [cache_prefix, &["pull"]].concat();
    let status_args: Vec<&str> = [cache_prefix, &["status", "--json"]].concat();
