    ...
    fn from_str(input: &str) -> Result<Parameter, Self::Err> {
        match input.split_once(':') {
            ...
            Some((key, value)) => match (key.len(), value.len()) {
                (key_len, _) if key_len == 0 => Err("parameter key cannot be empty"),
                (key_len, _) if key_len > 255 => Err("parameter key cannot be longer than 255 bytes"),
...
impl ParameterMap {
    ...
    ///     (ParameterKey(String::from("key-a")), ParameterValue(String::from("value-a"))),
    ///     (ParameterKey(String::from("key-b")), ParameterValue(String::from("value-b"))),
    ///   ]))),
    ...
    ///
    /// assert_eq!(
    ///   Err(String::from("Parameter key: key-a is present multiple times")),
    ///   ParameterMap::parse(&parameters)
    /// );
    /// ```
    pub fn parse<'a>(values: impl IntoIterator<Item = &'a Parameter>) -> Result<ParameterMap, String> {
        ...
        for parameter in values {
            if map.contains_key(&parameter.key) {
                return Err(format!("Parameter key: {} is present multiple times", parameter.key.0))
            } else {
    ...
    ///     (ParameterKey(String::from("key-a")), ParameterValue(String::from("value-a"))),
    ///     (ParameterKey(String::from("key-b")), ParameterValue(String::from("value-b1"))),
    /// ]));
    ///
    /// let extra = ParameterMap(std::collections::BTreeMap::from([
    ///     (ParameterKey(String::from("key-b")), ParameterValue(String::from("value-b2"))),
    ///     (ParameterKey(String::from("key-c")), ParameterValue(String::from("value-c"))),
    /// ]));
    ///
    /// assert_eq!(
