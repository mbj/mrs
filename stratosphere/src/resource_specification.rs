...
macro_rules! identifier {
    ...
    ($struct: ident, $pattern: literal) => {
        ...
        impl<'a> std::convert::TryFrom<&'a str> for $struct<'a> {
            ...
            fn try_from(value: &'a str) -> Result<$struct<'a>, Self::Error> {
...
#[derive(Debug, serde::Deserialize)]
pub struct Documentation<'a>(&'a str);
...
#[derive(Debug, serde::Deserialize)]
pub struct ResourceSpecificationVersion<'a>(&'a str);
...
impl<'a, 'de: 'a> serde::Deserialize<'de> for ResourceTypeName<'a> {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        <&'a str as serde::de::Deserialize<'de>>::deserialize(deserializer).and_then(|value| {
            ...
            if parts.len() == 3 {
                Ok(ResourceTypeName {
                    vendor_name: VendorName(parts[0]),
                    service_name: ServiceName(parts[1]),
                    resource_name: ResourceName(parts[2]),
                })
            } else {
                Err(serde::de::Error::custom(format!("Invalid value: {value}")))
            }
        })
    }
}
...
impl std::fmt::Display for ResourceTypeName<'_> {
    fn fmt(&self, formatter: &mut std::fmt::Formatter) -> Result<(), std::fmt::Error> {
        write!(
            formatter,
            "{}::{}::{}",
            self.vendor_name, self.service_name, self.resource_name
        )
    }
}
