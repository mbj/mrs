...
use crate::secrets::{GitHubApp, WebhookSecret};
...
pub async fn run() {
    ...
    loop {
        let event = client.read_next_event::<serde_json::Value>().await;
        let response = match &event.body {
            Ok(body) => {
                log::info!("Received event: {}", serde_json::to_string_pretty(body).unwrap());
                Response::json(http::StatusCode::OK, body)
            }
            Err(error) => {
                log::error!("Failed to decode event: {}", error.message());
                Response::error(http::StatusCode::BAD_REQUEST, &error.message())
            }
        };
